...
const cards = [
  ...
  return (
    <div className="w-fit h-fit rounded-xl grid xs:grid-cols-2 gap-4 xl:mx-auto max-w-3xl   overflow-auto">
      {cards.map((card, index) => (
        <div
          key={index}
          className="bg-white h-full max-w-[300px] xl:max-w-[400px] rounded-xl shadow-lg flex flex-col  justify-center p-4 pl-6  hover:bg-blue-200  transition-all duration-300  hover:shadow-xl border border-gray-100 aspect-square min-w-0 flex-shrink"
        >
          <article className="w-full flex gap-2 justify-start items-center">
            <div className="text-2xl text-indigo-600">{card.icon}</div>
            <h3 className="my-3 text-md font-semibold text-gray-900 px-3 leading-tight">{card.title}</h3>
          </article>
          <p className="text-left h-fit text-gray-900 leading-tight">{card.description}</p>
        </div>
      ))}
    </div>
  );
